...
class TestTopLevelFunctions(unittest.TestCase):
    ...
    def test_int_to_roman_with_exception1(self):
        """
        number = 137438953472 (mutated from zero)
        """
        """This execution may take longer than the 1000 ms timeout
         and therefore fail due to exceeding the timeout."""
        conversions.roman_numerals.int_to_roman(137438953472)
    
    @unittest.skip(reason='Disabled due to the fact that the execution is longer then 1000 ms')
    def test_int_to_roman_with_exception2(self):
        """
        number = 17592186044417 (mutated from positive)
        """
        """This execution may take longer than the 1000 ms timeout
         and therefore fail due to exceeding the timeout."""
        conversions.roman_numerals.int_to_roman(17592186044417)
    # endregion
