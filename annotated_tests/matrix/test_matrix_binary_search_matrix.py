...
class TestTopLevelFunctions(unittest.TestCase):
    ...
    def test_binary_search2(self):
        """
        array = builtins.list[builtins.bool]
        lower_bound = -1
        upper_bound = -2 (mutated from -1)
        value = zero (mutated from min)
        """
        actual = matrix.binary_search_matrix.binary_search([False, False], -1, -2, 0)
    ...
    def test_binary_search_with_exception(self):
        ...
        lower_bound = 170141183460469231731687303715884105726 (mutated from -2)
        upper_bound = 3 (mutated from 2)
        value = 3 (mutated from 2)
        """
        # This test fails because function [matrix.binary_search_matrix.binary_search] produces [IndexError]
        matrix.binary_search_matrix.binary_search([], 170141183460469231731687303715884105726, 3, 3)
